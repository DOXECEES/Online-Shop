...
const ProductCard = ({ id, image, title, specifications, price }) => {
    ...
    const handleAddToCart = async (itemId) => {
        ...
        axios.post('http://localhost:5000/api/shopping-cart/add', {
            email: user.email,
            productID: itemId,
            quantity: 1
        })
    ...
    const handleMoreClick = () => {
        //if (isChoosen) {
        goods.addItemToBasket(id);
        //setIsChoosen(true);
        //}
        //else {
        //  goods.deleteItemFromBasket(id);
        //setIsChoosen(false);
        //}
    };
    ...
    return (
        <div className="product-card">
            ...
            <div className="product-footer">
                <button className="btn-more" onClick={() => handleAddToCart(id)}>{"Купить"}</button>
                <span className="product-price">{price} р.</span>
            </div>
        </div>
    );
};
